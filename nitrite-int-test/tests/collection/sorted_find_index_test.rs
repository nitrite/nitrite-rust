...
/// fat collection costs many times the lean one. A sorted page that decodes only the rows it
/// returns costs the same either way.
///
/// Deliberately not "sorted page vs. full drain": both halves here are measured back to back
/// under whatever load the machine is under, and the only variable between them is the thing
/// that was broken.
#[test]
...
fn sorted_page_cost(
    ...
    let options = order_by("seq", SortOrder::Descending).limit(20);
